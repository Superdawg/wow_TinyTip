...
if not modulecore then
  ...
  if (not reason or reason ~= "MISSING") and not IsAddOnLoaded(name) then
    return
  end -- skip internal loading if module is external
...
function module.TooltipFormat(unit, name, realm, isPlayer, isPlayerOrPet, isDead)
    ...
    -- print("Unit: " .. unit)
    -- if unit == "target" then
    --   print("Unit is target")
    --   print("UnitIsTapped(unit) = " .. UnitIsTapped(unit))
    --   print("UnitIsTappedByPlayer(unit) = " .. UnitIsTappedByPlayer(unit))
    -- end
    if (unit == "target" and (UnitIsEnemy(unit, "player") and not UnitIsPlayer(unit))) then
        bdR, bdG, bdB = 0.54, 0.54, 0.54
        GameTooltipTextLeft1:SetTextColor(0.54, 0.54, 0.54)
        deadOrTappedColour = "888888"
    elseif (unit == "target" and ((UnitIsDead(unit)) or (UnitIsEnemy(unit, "player") and UnitIsTapped(unit) and not UnitIsTappedByPlayer(unit)))) then
        bdR, bdG, bdB = 0.54, 0.54, 0.54
        GameTooltipTextLeft1:SetTextColor(0.54, 0.54, 0.54)
        deadOrTappedColour = "888888"
    elseif unit == "target" and UnitCanAttack(unit, "player") and (( isPlayerOrPet and UnitCanAttack(unit, "player") ) or UnitIsTappedByPlayer(unit) or
    ...
    elseif unit == "target" and (( isPlayerOrPet and UnitCanAttack("player", unit) ) or
    ...
    if levelLine then
        ...
        if levelDiff and UnitFactionGroup(unit) ~= UnitFactionGroup("player") then
            ...
            elseif -levelDiff <= UnitQuestTrivialLevelRange(unit) then levelColour = "00FF00"
...
if not modulecore then
    ...
    local function OnTooltipSetUnit(self, ...)
        ...
        if (unit and not (unit == nil) and UnitExists(unit) and GameTooltipTextLeft1:IsShown()) then
